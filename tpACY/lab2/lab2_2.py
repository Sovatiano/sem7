...
x_name = 'Квалификация персонала'
y_name = 'Качество материальной базы'
z_name = 'Качество выполненной работы'

x_var = fuzz.control.Antecedent(np.arange(0, 100, 0.5), x_name)
x_var['Низкая'] = fuzz.gaussmf(x_var.universe, 15, 10)
x_var['Средняя'] = fuzz.gaussmf(x_var.universe, 50, 10)
x_var['Высокая'] = fuzz.gaussmf(x_var.universe, 85, 10)
# profit.view()

y_var = fuzz.control.Antecedent(np.arange(0, 100, 0.5), y_name)
y_var['Низкое'] = fuzz.gaussmf(y_var.universe, 15, 10)
y_var['Среднее'] = fuzz.gaussmf(y_var.universe, 50, 10)
y_var['Высокое'] = fuzz.gaussmf(y_var.universe, 85, 10)
# term.view()
...
probbing.input[x_name] = 95
probbing.input[y_name] = 40
